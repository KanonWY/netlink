...
void initSerial(serial::Serial *serial) {
  try {
    ...
    printf("try to open serial port with %s,%d", port_name.data(), baud_rate);
    ...
    if (serial->isOpen()) {
      ...
      printf("Serial port opened successfully, waiting for data.");
    } else {
      // ROS_ERROR("Failed to open serial port, please check and retry.");
      std::cerr << "Failed to open serial port, please check and retry." << std::endl;
      exit(EXIT_FAILURE);
    }
